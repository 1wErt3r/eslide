...
static void _net_position_label(void)
{
    if (!net_label || !letterbox_bg)
        return;
    int x, y, w, h;
...
static void _net_set_text(const char* txt)
{
    if (!net_label || !txt)
        return;
    char buf[1024];
    ...
    snprintf(buf, sizeof(buf),
        "<font=Open Sans:style=Light><font_size=24><color=#FFFFFF>%s</color></font_size></font>",
        txt);
...
static Eina_Bool _url_data_cb(void* data EINA_UNUSED, int type EINA_UNUSED, void* event)
{
...
static Eina_Bool _url_complete_cb(void* data EINA_UNUSED, int type EINA_UNUSED, void* event)
{
...
static void _net_on_letterbox_resize(void* data EINA_UNUSED, Evas* e EINA_UNUSED,
...
void net_init(Evas_Object* parent_window)
{
    ...
    if (letterbox_bg)
        evas_object_event_callback_add(
            letterbox_bg, EVAS_CALLBACK_RESIZE, _net_on_letterbox_resize, NULL);
}
...
void net_fetch_start(void)
{
    ...
    if (net_station && *net_station) {
        ...
        snprintf(
            url_buf, sizeof(url_buf), "https://wttr.in/%s?format=1", eina_strbuf_string_get(tmp));
...
static Eina_Bool _net_timer_cb(void* data EINA_UNUSED)
{
...
void net_cleanup(void)
{
    ...
    if (letterbox_bg) {
        evas_object_event_callback_del(
            letterbox_bg, EVAS_CALLBACK_RESIZE, _net_on_letterbox_resize);
    }
    if (net_timer) {
        ecore_timer_del(net_timer);
        net_timer = NULL;
...
const char* net_get_station(void)
{
    return net_station;
}