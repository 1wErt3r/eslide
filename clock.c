...
Eina_Bool clock_timer_cb(void* data EINA_UNUSED)
{
    time_t current_time;
    struct tm* time_info;
    ...
    void* data EINA_UNUSED, Evas* e EINA_UNUSED, Evas_Object* obj, void* event_info EINA_UNUSED)
{
    ...
    if (cw <= 0)
        cw = 520; // fallback to default size if not yet realized
    if (ch <= 0)
        ch = 180;
...
void clock_init(Evas_Object* parent_window)
{
    ...
    clock_label
        = elm_label_add(parent_window); // Add to window instead of letterbox for proper layering
